...
- `dismissed` / `fp_reason` / `injected_count` / `last_injected` カラム追加済み（Phase 1 ALTER TABLE 完了）
- インデックス `idx_file_path` / `idx_pending` 作成済み
...
### Phase 2（完了）
- [x] `hooks/post-tool-edit-counter.py` 実装（バッチカウント）
- [x] `hooks/session-end-learn.py` 実装（プロジェクト固有 CLAUDE.md に追記）
- [x] `scripts/batch-review-trigger.py` 実装（5編集ごとのバッチレビュー起動）
