...
- `dismissed` / `fp_reason` / `injected_count` / `last_injected` は Phase 1 で ALTER TABLE 追加予定
- **Phase 1 前（ALTER TABLE 未実施）**: hook はエラーなく起動するが findings は一切注入されない（OperationalError を黙って握りつぶして空リストを返す設計）
...
- [ ] `hooks/session-end-learn.py` 実装（プロジェクト固有 CLAUDE.md に追記）
- [ ] `scripts/batch-review-trigger.py` 実装（5編集ごとのバッチレビュー起動）
